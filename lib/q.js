...
* @param {Number} max time (minutes) a job is allowed to spend in active state
*/

q.watchStuckActiveJobs = function (sec, max) {

  sec = sec || 5 * 60;
  max = max || 5;

  setInterval(watch, sec * 1000);
      ...
      var tasks = ids.map(function (id) {
        return function (next) {
          kue.Job.get(id, function (error, job) {
            ...
            if ( moment(+job.updated_at).isBefore(moment().subtract(max, 'minutes')) ) {
